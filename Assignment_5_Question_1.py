...
state_capitals_list = list(state_capitals_dict)

answer = ''
correct = 0
incorrect = 0

while (answer:=input(f'What is the capital of {state_capitals_list[number:=rand.randint(0,49)]}? (or enter q to quit): ')) != 'q':
    if answer.lower() == (state_capitals_dict[state_capitals_list[number]]).lower():
        print ('That is correct.')
        correct += 1
    else:
        print('That is incorrect.')
        incorrect += 1
print(f'You had {correct} correct answers and {incorrect} incorrect answers.')
